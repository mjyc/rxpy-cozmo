...
def main(sources):
    ...
    example3_completed = rx.combine_latest(
        ...
        sources["Cozmo"].pipe(
            ops.filter(lambda i: i["id"]
                       is "example3_display_oled_face_image"))
    )
    example4_completed = rx.combine_latest(
        ...
        sources["Cozmo"].pipe(
            ops.filter(lambda i: i["id"]
                       is "example4_display_oled_face_image")
        ),
    )
    example1_completed.subscribe(
        on_next=lambda i: print("example1_completed"))
    example2_completed.subscribe(
        on_next=lambda i: print("example2_completed"))
    example3_completed.subscribe(
        on_next=lambda i: print("example3_completed"))
    example4_completed.subscribe(
        on_next=lambda i: print("example4_completed"))
    rxcozmo = rx.merge(
        # example1_lift_head
        rx.of({
        ...
        example1_completed.pipe(ops.map(lambda i: {
        ...
        example2_completed.pipe(ops.map(lambda i: {
        ...
        example2_completed.pipe(ops.map(lambda i: {
            ...
            "name": "set_head_angle",
            "value": {"angle": cozmo.robot.MIN_HEAD_ANGLE, "duration": 6.0}
        })),
        example2_completed.pipe(ops.map(lambda i: {
        ...
        example3_completed.pipe(ops.map(lambda i: {
